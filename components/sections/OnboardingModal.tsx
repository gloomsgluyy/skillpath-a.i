...
function getAuthErrorMessage(error: unknown, fallback: string) {
  ...
  if (code === 'auth/unauthorized-domain') {
    return `Domain ${host} belum diizinkan untuk login Google di Firebase Authentication. Tambahkan domain ini di Authorized domains.`;
  }

  if (code === 'auth/popup-closed-by-user') {
    return 'Popup Google ditutup sebelum proses login selesai.';
  }
  ...
  if (code === 'auth/email-already-in-use') {
    return 'Email ini sudah terdaftar. Coba masuk dengan password yang benar.';
  }
...
export default function OnboardingModal() {
  ...
  return (
    <div className="bg-white/90 backdrop-blur-2xl rounded-2xl sm:rounded-[2.5rem] overflow-hidden shadow-[0_32px_80px_-20px_rgba(0,0,0,0.15)] relative w-full max-w-none min-w-0 flex flex-col border border-white/40" style={{ maxHeight: "calc(100dvh - 1rem)" }}>
      {/* Header */}
        ...
        <AnimatePresence mode="wait">
          ...
          {data.step === 5 && (
            <motion.div 
              ...
              className="flex w-full min-w-0 flex-col items-center justify-center text-center py-4"
            >
              ...
              <div className="mx-auto flex w-full max-w-sm flex-1 flex-col gap-3 overflow-y-auto min-h-0 pr-1 mb-4">
                <input 
                  type="text"
              ...
              <div className="mx-auto w-full max-w-sm shrink-0">
               ...
               >
                ...
                {authError && (
                  <div className="mt-3 flex gap-2 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-left text-xs font-bold leading-relaxed text-red-700">
